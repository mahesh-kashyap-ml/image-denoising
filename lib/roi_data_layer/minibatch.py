...
def _get_image_blob(roidb, scale_inds):
  ...
  for i in range(num_images):
    ...
    def add_poisson_noise(noise_type):
        ...
        #also called shot noise originates from the discrete nature of electronic charge or photons.
            if ('poisson_wavelet' in noise_type):
                ...
                pois_array = random_noise(img, mode='poisson')
                #pois_array = np.random.poisson(img)
                im = pois_array
        ...
        if ('sap' in noise_type):
            ...
            if ('sap_wavelet' in noise_type):
                ...
                if ('var0.2' in noise_type):
                    ...
                    sp_array = random_noise(img, mode='s&p', amount=0.6)
                    #im = (255 * sp_array).astype(np.uint8)
                    im = sp_array
                    print('s&p var 0.8')
            print("salt & pepper")
        ...
        if('uniform' in noise_type):
            ...
            if ('uniform_wavelet' in noise_type):
                ...
                if ('var0.2' in noise_type):
                    ...
                    im = cv2.GaussianBlur(im_noise, (size, size), 0)
                    print('uniform gaus blur var 0.2')
                elif ('var0.6' in noise_type):
                    uniform_array = np.random.uniform(low=0., high=0.6, size=img.shape)
                    uniform_noise = cv2.add(image, uniform_array)
                    im_noise = (255 * uniform_noise).astype(np.uint8)
                    im = cv2.GaussianBlur(im_noise, (size, size), 0)
                    print('uniform gaus blur var 0.6')
                elif ('var1.2' in noise_type):
                    uniform_array = np.random.uniform(low=0., high=1.2, size=img.shape)
                    uniform_noise = cv2.add(image, uniform_array)
                    im_noise = (255 * uniform_noise).astype(np.uint8)
                    im = cv2.GaussianBlur(im_noise, (size, size), 0)
                    print('uniform gasu blur var 1.2')
    ...
    if ('mix' in roidb[i]['noise_type']):
        if ('var_low' in roidb[i]['noise_type']):
            noise_list = ['gaussian_var0.1', 'poisson', 'speckle_var0.5',
                            ...
                            'rayleigh_var0.1','periodic_var3.14','brownian_var0.9', 'quant_var10', 'original', 'bloom', 'shader']
            ...
            noise_list = ['gaussian_var1.0', 'poisson', 'speckle_var1.0',
                            ...
                            'rayleigh_var0.2','periodic_var100','brownian_var0.09', 'quant_var7', 'original', 'bloom', 'shader']
            """noise_list = ['gaussian_var1.0', 'poisson', 'speckle_var1.0',
            ...
            noise_list = ['gaussian_var1.5', 'poisson', 'speckle_var2.0', 
                            ...
                            'rayleigh_var0.3','periodic_varsize','brownian_var0.009', 'quant_var3', 'original', 'bloom', 'shader']
            noise_type = random.choice(noise_list)
        elif ('var_all' in roidb[i]['noise_type']):
            noise_list = ['gaussian_var0.1', 'poisson', 'speckle_var0.5',
                            'sap_var0.2', 'uniform_var0.2', 'gamma_var0.05',
                            'rayleigh_var0.1','periodic_var3.14','brownian_var0.9', 'quant_var3', 'shader', 'bloom',
                            'gaussian_var1.0', 'poisson', 'speckle_var1.0',
                            'sap_var0.4', 'uniform_var0.6', 'gamma_var0.1', 'original', 'shader', 'bloom', 
        ...
        noise_type = roidb[i]['noise_type']
        im = add_shader()
    elif('curvelet' in roidb[i]['noise_type']):
        print('curvelet')
    else :
        im = retain_original()
    ...
    if ('gaus_blur' in roidb[i]['noise_type']):
        ...
        img = subprocess.check_output(["python3", "/home/mahesh/thesis/de-noise/tf-faster-rcnn/lib/roi_data_layer/fdct.py", noise_type, img_path])
  ...
  #print(num_images) processed_ims size is 1, as only 1 image will be used to train in a batch, later 128 roi will be selected from this.
  #print(processed_ims)
